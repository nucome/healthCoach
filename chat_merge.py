...
<body>
    <div class="container">
        ...
        <div class="responses-grid">
            ...
            <div class="model-card" id="card-anthropic">
                <div class="model-header">
                    <input type="checkbox" class="model-checkbox" id="checkbox-anthropic" checked>
                    <div class="model-info">
            ...
            <div class="model-card" id="card-openai">
                <div class="model-header">
                    <input type="checkbox" class="model-checkbox" id="checkbox-openai" checked>
                    <div class="model-info">
    ...
    <script>
        ...
        // Handle layout based on checkbox states
        function updateLayout() {
            const ollamaChecked = document.getElementById('checkbox-ollama').checked;
            const anthropicChecked = document.getElementById('checkbox-anthropic').checked;
            const openaiChecked = document.getElementById('checkbox-openai').checked;
            const googleChecked = document.getElementById('checkbox-google').checked;

            const ollamaCard = document.getElementById('card-ollama');
            const anthropicCard = document.getElementById('card-anthropic');
            const openaiCard = document.getElementById('card-openai');
            const googleCard = document.getElementById('card-google');

            const ollamaCheckbox = document.getElementById('checkbox-ollama');
            const anthropicCheckbox = document.getElementById('checkbox-anthropic');
            const openaiCheckbox = document.getElementById('checkbox-openai');
            const googleCheckbox = document.getElementById('checkbox-google');

            // Reset all cards
            [ollamaCard, anthropicCard, openaiCard, googleCard].forEach(card => {
                card.classList.remove('hidden', 'expanded', 'collapse-checkbox');
            });

            // Row 1: DeepSeek & Claude - merge if exactly one is unchecked
            if (ollamaChecked && !anthropicChecked) {
                // Only DeepSeek - expand it across both columns
                ollamaCard.classList.add('expanded');
                anthropicCard.classList.add('hidden');
            } else if (!ollamaChecked && anthropicChecked) {
                // Only Claude - expand it across both columns
                anthropicCard.classList.add('expanded');
                ollamaCard.classList.add('hidden');
            } else if (!ollamaChecked && !anthropicChecked) {
                // Both unchecked - hide both
                ollamaCard.classList.add('hidden');
                anthropicCard.classList.add('hidden');
            }

            // Row 2: GPT-5 & Gemini - merge if exactly one is unchecked
            if (openaiChecked && !googleChecked) {
                // Only GPT-5 - expand it across both columns
                openaiCard.classList.add('expanded');
                googleCard.classList.add('hidden');
            } else if (!openaiChecked && googleChecked) {
                // Only Gemini - expand it across both columns
                googleCard.classList.add('expanded');
                openaiCard.classList.add('hidden');
            } else if (!openaiChecked && !googleChecked) {
                // Both unchecked - hide both
                openaiCard.classList.add('hidden');
                googleCard.classList.add('hidden');
